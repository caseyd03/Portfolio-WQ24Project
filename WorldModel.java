...
public final class WorldModel {
    ...
    public Optional<TargetEntity> findNearest(Point pos, List<Class> classes) {
        List<TargetEntity> ofType = new LinkedList<>();
        for (Class kind : classes) {
            for (Entity entity : entities) {
                if (entity.getClass() == kind) {
                    ofType.add((TargetEntity) entity);
                }
            }
        }
    ...
    public Optional<Entity> findEnemy(Point pos, List<Class> classes) {
        ...
        return nearestEnemy(ofType, pos);
    }

    private static Optional<TargetEntity> nearestEntity(List<TargetEntity> entities, Point pos) {
        if (entities.isEmpty()) {
            return Optional.empty();
        } else {
            TargetEntity nearest = entities.get(0);
            int nearestDistance = distanceSquared(nearest.getPosition(), pos);

            for (TargetEntity other : entities) {
    ...
    private static Optional<Entity> nearestEnemy(List<Entity> entities, Point pos) {
        if (entities.isEmpty()) {
            return Optional.empty();
        } else {
